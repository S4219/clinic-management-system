...
const STATUS_BADGE = {
  ...
  overdue: 'badge-red', partial: 'badge-blue',
}
...
export default function ReceptionistBills() {
  ...
  const [modal,    setModal]    = useState(null)   // 'generate' | 'edit'
  ...
  async function handleSave(e) {
    ...
    if (modal === 'generate' && !selected.bill_id.trim()) { toast.error('Bill ID is required'); return }
    if (!selected.patient_id)   { toast.error('Select a patient'); return }
    if (tot <= 0)                 { toast.error('Enter at least one charge amount'); return }

    setSaving(true)
    const payload = {
      ...(modal === 'generate' ? { bill_id: selected.bill_id } : {}),
      patient_id:       selected.patient_id,
      appointment_id:   selected.appointment_id || null,
      invoice_number:   modal === 'generate' ? invoiceNum() : selected.invoice_number,
    ...
    <ReceptionistLayout>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="page-sub">{total} invoices total</p>
        </div>
        <button
          onClick={() => { setSelected(BILL_EMPTY); setModal('generate') }}
                ...
                : rows.length === 0
                  ...
                  : rows.map(r => (
                      <tr key={r.bill_id} className="hover:bg-gray-50/60 transition-colors">
                        ...
                        <td className="td">
                          <span className={STATUS_BADGE[r.payment_status] ?? 'badge-gray'}>
                            {r.payment_status}
                          </span>
                        </td>
                        <td className="td">
                          <div className="flex gap-1">
                            ...
                            <button
                              onClick={() => printBill(r)}
                              className="btn-ghost p-1.5 text-purple-500 hover:text-purple-700 hover:bg-purple-50"
                              title="Print"
                            >
      ...
      {modal && (
        <ModalShell
          title={modal === 'generate' ? 'Generate Bill' : 'Edit Bill'}
          subtitle={modal === 'edit' ? selected.bill_id : undefined}
          ...
          <form onSubmit={handleSave} className="space-y-4">
            {modal === 'generate' && (
              <div>
                ...
                <input className="input" value={selected.bill_id}
            ...
            <div>
              ...
              <select className="input" value={selected.appointment_id || ''}
                onChange={e => set('appointment_id', e.target.value)}
                disabled={!selected.patient_id}>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select className="input" value={selected.payment_status}
              ...
              <div>
                ...
                <select className="input" value={selected.payment_method || ''}
            ...
            <ModalActions onClose={() => setModal(null)} loading={saving}
              submitLabel={modal === 'generate' ? 'Generate Bill' : 'Save Changes'} />
          </form>
        </ModalShell>
      )}
    </ReceptionistLayout>
